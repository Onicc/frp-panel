  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="show" class="modal-backdrop" role="dialog" aria-modal="true" @click.self="closeOnClickOutside && emit('close')">
        <section class="modal" :class="widthClass" @click.stop>
          <header class="modal-header"><h2>{{ title }}</h2><button v-if="showCloseButton" class="icon-button" type="button" aria-label="Close" @click="emit('close')"><Icon name="x" /></button></header>
