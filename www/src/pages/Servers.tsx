...
export default function Servers() {
  ...
  return <>
    <header className="page-header"><div><span className="eyebrow">{t('dataPlane')}</span><h1>{t('servers')}</h1><p>{t('serverTopologyHint')}</p></div><div className="header-actions"><button className="button secondary" onClick={() => void refresh()}>{t('refresh')}</button><button className="button primary" onClick={() => setOpen(true)}>＋ {t('createServer')}</button></div></header>
    {deployment && <section className="install-card" aria-live="polite"><div className="panel-heading"><div><span className="step-badge">{t('nextStep')}</span><h2>{t('deployServer')}</h2><p>{t('serverTokenHint')}</p></div><button className="icon-button" aria-label={t('close')} onClick={() => setDeployment(null)}>×</button></div><pre><code>{compose}</code></pre><button className="button secondary" onClick={async () => { await navigator.clipboard.writeText(compose); setCopied(true); window.setTimeout(() => setCopied(false), 1500) }}>{copied ? t('copied') : t('copyCompose')}</button></section>}
    {error && <p className="error alert" role="alert">{error}</p>}
    <section className="panel table-panel"><div className="table-title"><div><h2>{t('serverInventory')}</h2><p>{t('serverInventoryHint')}</p></div><span className="count-badge">{servers.length}</span></div><div className="table-scroll"><table><thead><tr><th>{t('serverId')}</th><th>{t('serverIp')}</th><th>{t('status')}</th></tr></thead><tbody>{servers.map((server) => <tr key={server.id ?? server.serverId}><td><div className="resource-name"><span className="resource-icon server-icon">S</span><strong>{server.id ?? server.serverId}</strong></div></td><td>{server.ip || '—'}</td><td><span className={server.online ? 'status online' : 'status offline'}><i />{server.online ? t('online') : t('offline')}</span></td></tr>)}</tbody></table></div>{loading && <div className="empty">{t('loading')}</div>}{!loading && servers.length === 0 && <div className="empty"><strong>{t('noServers')}</strong><span>{t('noServersHint')}</span></div>}</section>
