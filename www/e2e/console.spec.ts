...
async function mockAPI(page: import('@playwright/test').Page) {
  ...
  await page.route('**/api/v2/updates/release**', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ supported: true, release: { channel: 'stable', currentVersion: 'v1.0.0', currentCommit: 'a'.repeat(40), latestVersion: 'v1.1.0', latestCommit: 'b'.repeat(40), releaseUrl: 'https://github.com/Onicc/frp-panel/releases/tag/v1.1.0', available: true, checkedAt: new Date().toISOString() } }) }))
...
test('version controls show a local Client command and editable Server maintenance window', async ({ page }) => {
  ...
  await expect(page.locator('.version-popover-header')).toContainText('当前版本')
...
test('Master version menu matches the sub2api badge, dropdown and update-card layout', async ({ page }, testInfo) => {
  await mockAPI(page)
  await page.goto('/login')
  await page.getByLabel('用户名').fill('owner')
  await page.getByLabel('密码').fill('correct-password')
  await page.getByRole('button', { name: '继续' }).click()
  const badge = page.locator('.version-pill')
  await expect(badge).toHaveText('v1.0.0')
  await expect(badge).toHaveCSS('border-radius', '8px')
  await badge.click()
  const menu = page.locator('.version-popover')
  await expect(menu).toHaveCSS('width', '256px')
  await expect(menu.locator('.version-popover-header')).toContainText('当前版本')
  await expect(menu.locator('.version-current-value strong')).toHaveText('v1.0.0')
  await expect(menu.locator('.version-current-value strong')).toHaveCSS('font-size', '24px')
  await expect(menu.locator('.version-current p')).toHaveText('最新版本: v1.1.0')
  await expect(menu.locator('.version-notice-update')).toContainText('v1.1.0')
  await expect(menu.getByRole('button', { name: '立即更新' })).toBeVisible()
  await expect(menu.getByRole('link', { name: '查看发布说明' })).toHaveAttribute('href', 'https://github.com/Onicc/frp-panel/releases/tag/v1.1.0')
  await expect(menu).toHaveCSS('opacity', '1')
  const badgeBounds = await badge.boundingBox()
  const menuBounds = await menu.boundingBox()
  expect(badgeBounds && menuBounds && menuBounds.y > badgeBounds.y + badgeBounds.height).toBe(true)
  await page.screenshot({ path: testInfo.outputPath('version-menu-light.png') })

  await page.evaluate(() => document.documentElement.classList.add('dark'))
  await expect(menu).toBeVisible()
  await page.screenshot({ path: testInfo.outputPath('version-menu-dark.png') })
  await badge.click()
  ...
  expect(mobileBounds && mobileBounds.x >= 0 && mobileBounds.x + mobileBounds.width <= 390).toBe(true)
  await page.screenshot({ path: testInfo.outputPath('version-menu-mobile.png') })
})

test('legacy edge builds are labeled honestly and require manual migration', async ({ page }) => {
  await mockAPI(page)
  const edgeVersion = { ...oldVersion, gitVersion: 'edge-SNAPSHOT-7cd4e60' }
  await page.route('**/api/v2/updates/release**', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ supported: true, release: route.request().url().includes('channel=stable') ? { channel: 'stable', currentVersion: edgeVersion.gitVersion, currentCommit: edgeVersion.gitCommit, latestVersion: 'v2.1.0', latestCommit: 'b'.repeat(40), available: null, checkedAt: new Date().toISOString() } : { channel: 'legacy', currentVersion: edgeVersion.gitVersion, currentCommit: edgeVersion.gitCommit, available: null, error: 'manual migration required', checkedAt: new Date().toISOString() } }) }))
  await page.route('**/api/v2/clients**', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ items: [{ ...client, version: edgeVersion }], total: 1, page: 1, pageSize: 25 }) }))
  await page.route('**/api/v2/servers**', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ items: [{ ...server, version: { ...edgeVersion, platform: 'linux/amd64' } }], total: 1, page: 1, pageSize: 25 }) }))
  await page.goto('/login')
  await page.getByLabel('用户名').fill('owner')
  await page.getByLabel('密码').fill('correct-password')
  await page.getByRole('button', { name: '继续' }).click()
  await expect(page.locator('.version-pill')).toHaveText('旧测试版')
  await expect(page.locator('.version-pill')).toHaveAttribute('title', '旧测试版 · aaaaaaa')
  await page.locator('.version-pill').click()
  await expect(page.locator('.version-popover')).toContainText('迁移到正式版')
  await expect(page.locator('.version-popover')).not.toContainText('edge-SNAPSHOT')
  await expect(page.locator('.version-popover .version-action')).toHaveCount(0)
  await page.keyboard.press('Escape')
  await page.getByRole('complementary', { name: 'Primary navigation' }).getByRole('link', { name: 'Clients', exact: true }).click()
  await expect(page.locator('.version-cell').first()).toContainText('旧测试版')
  await expect(page.locator('.version-cell').first()).not.toContainText('7cd4e60')
  await expect(page.getByRole('button', { name: '迁移到正式版' })).toBeVisible()
  await page.getByRole('complementary', { name: 'Primary navigation' }).getByRole('link', { name: 'Servers', exact: true }).click()
  await expect(page.locator('.version-cell').first()).toContainText('旧测试版')
  await expect(page.locator('.version-cell').first()).not.toContainText('7cd4e60')
  await page.getByRole('button', { name: '迁移到正式版' }).click()
  await expect(page.getByRole('dialog')).toContainText('onicc/frp-panel:v2.1.0')
})

test('version menu presents failed-check retry and up-to-date states', async ({ page }, testInfo) => {
  await mockAPI(page)
  let failed = true
  await page.route('**/api/v2/updates/release**', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({
      supported: true,
      release: {
        channel: 'stable', currentVersion: 'v1.0.0', currentCommit: 'a'.repeat(40),
        latestVersion: failed ? undefined : 'v1.0.0',
        available: failed ? null : false,
        error: failed ? 'GitHub unavailable' : undefined,
  ...
  await page.locator('.version-pill').click()
  const menu = page.locator('.version-popover')
  await expect(menu.locator('.version-notice-error')).toContainText('GitHub unavailable')
  failed = false
  await menu.getByRole('button', { name: '重试' }).click()
  await expect(menu.locator('.version-current-check')).toBeVisible()
  await expect(menu.locator('.version-current p')).toHaveText('已是最新版本')
  await expect(menu.locator('.version-notice-error')).toHaveCount(0)
  await expect(menu.getByRole('button', { name: '立即更新' })).toHaveCount(0)
  await menu.getByRole('button', { name: '版本回退' }).click()
  await expect(menu.locator('.version-rollback-hint')).toContainText('网页暂不执行手动回退')
  await page.screenshot({ path: testInfo.outputPath('version-menu-up-to-date.png') })
})

test('non-Owner sees the concise static version, matching the reference sidebar', async ({ page }) => {
  await mockAPI(page)
  await page.route('**/api/v2/auth/login', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ user: { ...user, role: 'admin' } }) }))
  await page.goto('/login')
  await page.getByLabel('用户名').fill('owner')
  await page.getByLabel('密码').fill('correct-password')
  await page.getByRole('button', { name: '继续' }).click()
  await expect(page.locator('.version-static')).toHaveText('v1.0.0')
  await expect(page.locator('.version-pill')).toHaveCount(0)
  await expect(page.locator('.version-popover')).toHaveCount(0)
})
