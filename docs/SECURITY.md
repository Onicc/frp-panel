...
| Arbitrary download or unsafe self-update | High | Automatic workerd download/proxy behavior is disabled by default. Master/Server updates accept only same-channel official GitHub release assets, verify SHA-256, ELF architecture and embedded commit, then use an image-owned launcher to validate startup and roll back failures. Server auto-update is opt-in; Clients are updated locally, not by remote RPC. Docker images remain the authoritative fallback. |
