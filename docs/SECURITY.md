...
|---|---:|---|
| Shared/default secrets and exposed node credentials | Critical | Controller refuses secrets shorter than 32 characters; API models never serialize persistent credentials; enrollment stores only a token hash and long-lived node credentials are hashed at rest. |
...
|---|---|
| Controller HTTP has no mandatory built-in public certificate | Terminate HTTPS at a trusted reverse proxy; do not expose port 9000 directly. A future stable profile should refuse non-loopback HTTP without an explicit acknowledgement. |
...
| Embedded FRPS authentication secrets must be recoverable at runtime | They are excluded from API serialization but remain encrypted-volume/operator protected rather than application-encrypted. Add envelope encryption with an external KMS for hosted deployments. |
...
- Restrict controller and FRPS ingress with firewall rules; expose Web/API through HTTPS.
- Back up the database and test restore before upgrades. v2 does not migrate v1 data.
- Keep privileged Agent features disabled unless the node and operator trust boundary requires them.
