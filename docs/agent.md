...
控制台生成的引导命令先下载到临时目录、校验 SHA-256，再把 Client Agent 原子写入系统目录。服务命令行只包含配置文件路径，Client 凭据保存在权限为 `0600` 的配置中。

请从 **Clients → 添加 Client** 创建注册命令，选择 Linux、macOS 或 Windows 后复制完整命令到目标 Client 主机执行。不要从本文拼接注册命令：其中的一次性令牌只有 10 分钟有效，并在首次成功兑换后失效。
...
注册令牌在服务启动前就会被兑换。如果命令已经写入 `/etc/frp-panel/agent.yaml`，但随后在 systemd 安装或启动阶段失败，请保留该文件并直接重新执行原安装命令；安装器会在令牌无法再次兑换时，仅复用 Client ID、Master 地址均匹配的现有受保护配置。原命令已丢失时，可用现有配置修复安装：

```bash
curl -fsSL https://raw.githubusercontent.com/Onicc/frp-panel/main/install.sh | sudo bash -s -- --config /etc/frp-panel/agent.yaml
```

不要先执行 `uninstall --purge`，否则 Client 凭据会被删除，必须在 Master 中创建新 Client 并生成新命令。
