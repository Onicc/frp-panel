...
A Client may own multiple Tunnels and every Tunnel may use a different Server. Master stores each Tunnel as desired state and, when the Client is online and both Client and Server are enrolled, best-effort pushes the assembled FRPC configuration to the Client; Tunnels for the same pair share a connection. After a Tunnel is deleted, the current protocol sends a Client-level removal event only when that physical Client has no other active Tunnel. If the Client still serves another Server, pair-specific connection cleanup is not guaranteed. The underlying Client-to-Server connection is not a separate user-managed resource.

The public port listens on the selected Server (FRPS) and forwards to the service reachable by the selected Client. Business traffic never passes through Master. A Tunnel created while a Client is offline remains stored in Master, but the current Agent reconnect path does not automatically re-apply v2 Tunnel configuration; after the Client is online, edit or submit the Tunnel again to trigger delivery. Local ports may be `1–65535`; Server bind ports and remote ports must be `1024–65535`.
